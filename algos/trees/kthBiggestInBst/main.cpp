...
void  getKthBiggestNode( TreeNode *root, int& k, int& val ) {
    if ( root == nullptr ) return ;

    getKthBiggestNode( root->right, k, val );
    k--;
    ...
    if (  k==0 ) { 
        ...
        getKthBiggestNode( root->left, k, val );
    }
}
...
int main( int argc, const char* argv[] ) {
    ...
    printTree( twenty );
    int kthbiggest= -1;
    getKthBiggestNode( twenty, k, kthbiggest );
    cout << "kth biggest is " << kthbiggest << endl;
