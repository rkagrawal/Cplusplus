...
std::optional<std::vector<int>>  targetSum( int target, const std::vector<int>& numbers, std::map<int, std::optional<std::vector<int>>>& m ) {
	...
	std::optional<std::vector<int>> shortestarr;
	...
	for( int i=0; i< numbers.size(); i++ ) {
		...
		if(optVal) {
			...
			if( !shortestarr || (*shortestarr).size() > (*optVal).size() ) shortestarr = std::move(optVal);
		}
	}

	m.insert( std::make_pair( target, std::move(shortestarr) ) );
	return m[target];
}
